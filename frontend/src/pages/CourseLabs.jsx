...
import { FlaskConical, Clock, Play, CheckCircle, ArrowLeft } from 'lucide-react';
...
export default function CourseLabs() {
  ...
  const fetchData = async () => {
    try {
      const [labsRes, courseRes, progressRes] = await Promise.all([
      ...
      console.error('Failed to fetch course labs:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Layout>
      <div className="space-y-6">
        <div>
          <Link to="/courses" className="flex items-center gap-2 text-gray-400 hover:text-white mb-4">
            <ArrowLeft className="w-4 h-4" />
            Back to Courses
          </Link>
          <h1 className="text-2xl font-bold text-white">{course?.title || 'Course Labs'}</h1>
        ...
        {labs.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">No labs available for this course</p>
            <p className="text-gray-500 text-sm mt-2">Labs will appear once they are assigned to this course</p>
          </div>
        )}
